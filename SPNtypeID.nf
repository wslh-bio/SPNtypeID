...
process cg_pipeline {
  ...
  script:
    ...
    perl /cgqc/run_assembly_readMetrics.pl --fast --numcpus ${params.cg_pipeline_cpus} -e 2200000 ${name}_cgqc.fastq.gz > ${name}_qual.tsv
    """
}
...
process kraken {
  ...
  script:
  """
  kraken --db /kraken-database/minikraken_20171013_4GB --threads ${params.kraken_cpus} --paired ${reads[0]} ${reads[1]} > ${name}_raw.txt
  kraken-report --db /kraken-database/minikraken_20171013_4GB ${name}_raw.txt > ${name}_kraken.txt
  """
}
...
process seroba {
  ...
  mv ${name}/pred.tsv ${name}_pred.tsv
...
process results {
  ...
  input:
  //file multiqc_config
  file("*_qual.tsv") from step1_results.collect()
  file("*_kraken.txt") from step2_results.collect()
  file("*_pred.tsv") from step3_results.collect()
  ...
  class result_values:
      ...
      coverage = "NotRun"
      quality = "NotRun"
      pass_cov_quality = False
      percent_strep = "NotRun"
      percent_spn = "NotRun"
      secondgenus = "NotRun"
      percent_secondgenus = "NotRun"
      pass_kraken = False
      pred = "NotRun"
      comment=""
  ...
  with open("SPNtypeID_results.csv",'w') as csvout:
      ...
      for id in results:
          result = results[id]
          writer.writerow([result.id,result.coverage,result.quality,result.pass_cov_quality,result.percent_strep,result.percent_spn,result.secondgenus,result.percent_secondgenus,result.pass_kraken,result.pred,result.comment])
  """
}
